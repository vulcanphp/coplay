...
$this
    ...
    ->setupMeta([
        'title' => $info['name'] . ': ' . Configurator::$instance->get('title', 'CoPlay') . ' - Stream Free Movies & TV Series Online'
    ])
?>
...
<main class="container my-8">
    <div class="flex flex-col mb-8 md:flex-row items-center md:items-start text-center md:text-left">
        <?php if (isset($info['profile_path'])) : ?>
            <div class="flex-none">
                <img src="https://image.tmdb.org/t/p/w300/<?= $info['profile_path'] ?>" alt="profile image" class="w-60 rounded-md">
            </div>
        <?php endif ?>
        <div class="<?= isset($info['profile_path']) ? 'md:ml-8' : '' ?>">
            <h2 class="text-4xl mt-4 md:mt-0 font-semibold mb-1"><?= $info['name'] ?></h2>
            <div class="flex flex-wrap items-center justify-center md:justify-start text-gray-400 text-sm">
                ...
                <span class="ml-2"><?= str_replace(['<#>', '<##>'], [Time::dateFormat($info['birthday'] ?? ''), $info['place_of_birth'] ?? ''], translate('<#> in <##>')) ?></span>
            </div>
            <p class="text-gray-300 mt-8" x-data="{toggleMore: false}">
                <?php
                $words = explode(' ', $info['biography'] ?? '');
                if (count($words) >= 50) {
                    echo join(' ', array_slice($words, 0, 50));
                    echo '<span x-cloak x-show="!toggleMore">...</span>';
                    echo '<span x-cloak x-show="toggleMore" x-transition>';
                    echo ' ' . join(' ', array_slice($words, 50));
                    echo '</span>';
            ...
            <?php if (isset($info['also_known_as'])) : ?>
                <h4 class="font-semibold mt-8"><?= translate('Also Known As:') ?></h4>
                <p class="text-sm ml-[-0.25rem]">
                    <span class="px-1 bg-slate-700 inline-block rounded m-1">
                        <?= join('</span><span class="px-1 bg-slate-700 inline-block rounded m-1">', $info['also_known_as'] ?? []) ?>
                    </span>
                </p>
            <?php endif; ?>
        </div>
    </div>
    <?php $this->include('includes.collection', ['collection' => $collection]) ?>
</main>