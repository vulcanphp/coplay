<?php

use App\Core\Configurator;
?>
<section class="flex flex-row flex-wrap mt-4">
    <div class="w-full md:w-6/12">
        <div class="md:mr-3 px-2 py-3 flex rounded bg-primary-900 text-gray-300">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-3/12 pr-2 fill-current text-amber-400" viewBox="0 0 24 24">
                <path d="M20 3H4c-1.103 0-2 .897-2 2v14c0 1.103.897 2 2 2h16c1.103 0 2-.897 2-2V5c0-1.103-.897-2-2-2zm.001 6c-.001 0-.001 0 0 0h-.465l-2.667-4H20l.001 4zM9.536 9 6.869 5h2.596l2.667 4H9.536zm5 0-2.667-4h2.596l2.667 4h-2.596zM4 5h.465l2.667 4H4V5zm0 14v-8h16l.002 8H4z"></path>
                <path d="m10 18 5.5-3-5.5-3z"></path>
            </svg>
            <div class="w-9/12">
                <p class="font-semibold text-xl text-gray-200"><?= translate('Dynamic Player') ?></p>
                <p class="text-sm mt-1"><?= translate('Our ' . Configurator::$instance->get('title', 'CoPlay') . ' Player is fully Responsive and User-Friendly that Easy to Use.') ?></p>
            </div>
        </div>
    </div>
    <div class="w-full md:w-6/12 mt-6 md:mt-0">
        <div class="md:ml-3 px-2 py-3 flex rounded bg-primary-900 text-gray-300">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-3/12 pr-2 fill-current text-amber-400" viewBox="0 0 24 24">
                <path d="M12 16c1.671 0 3-1.331 3-3s-1.329-3-3-3-3 1.331-3 3 1.329 3 3 3z"></path>
                <path d="M20.817 11.186a8.94 8.94 0 0 0-1.355-3.219 9.053 9.053 0 0 0-2.43-2.43 8.95 8.95 0 0 0-3.219-1.355 9.028 9.028 0 0 0-1.838-.18V2L8 5l3.975 3V6.002c.484-.002.968.044 1.435.14a6.961 6.961 0 0 1 2.502 1.053 7.005 7.005 0 0 1 1.892 1.892A6.967 6.967 0 0 1 19 13a7.032 7.032 0 0 1-.55 2.725 7.11 7.11 0 0 1-.644 1.188 7.2 7.2 0 0 1-.858 1.039 7.028 7.028 0 0 1-3.536 1.907 7.13 7.13 0 0 1-2.822 0 6.961 6.961 0 0 1-2.503-1.054 7.002 7.002 0 0 1-1.89-1.89A6.996 6.996 0 0 1 5 13H3a9.02 9.02 0 0 0 1.539 5.034 9.096 9.096 0 0 0 2.428 2.428A8.95 8.95 0 0 0 12 22a9.09 9.09 0 0 0 1.814-.183 9.014 9.014 0 0 0 3.218-1.355 8.886 8.886 0 0 0 1.331-1.099 9.228 9.228 0 0 0 1.1-1.332A8.952 8.952 0 0 0 21 13a9.09 9.09 0 0 0-.183-1.814z"></path>
            </svg>
            <div class="w-9/12">
                <p class="font-semibold text-xl text-gray-200"><?= translate('Auto Update') ?></p>
                <p class="text-sm mt-1"><?= translate('We Crawl at the request and Serve Up-To-Date Streaming Links.') ?></p>
            </div>
        </div>
    </div>
</section>