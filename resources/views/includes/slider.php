...
if (isset($sliderItems) && !empty($sliderItems)): ?>
                            ...
                            <div class="w-10/12 md:w-8/12">
                                <div data-swiper-parallax="-200" data-swiper-parallax-duration="325">
                                    <h2 class="text-xl md:text-2xl lg:text-3xl font-semibold mb-4">
                                        <?= e($sliderItem['name'] ?? $sliderItem['title']) ?>
                                    </h2>
                                    ...
                                    <div class="flex flex-wrap justify-center md:justify-start gap-x-3 gap-y-1.5 mt-2">
                                        <?php foreach ($sliderItem['genres'] ?? [] as $id => $genre): ?>
                                            <a href="<?= route_url(
                                                ...
                                                [
                                                    'type' => $isTv ? 'tv' : 'movie',
                                                    'slug' => Str::slug($genre) . '-' . $id
                                                ]
                                            ) ?>"
                                                class="text-sm text-primary-100/85 hover:text-accent-400 transition"><?= e($genre) ?></a>
                                        <?php endforeach ?>
                                    </div>
                                    <div class="mb-5 mt-2">
                                        <p class="hidden md:block text-[0.8rem]">
                                            <?= Str::words($sliderItem['overview'] ?? '', 30) ?>
                                        </p>
                                    </div>
                                    <a href="<?= url(($isTv ? 'tv' : 'movie') . '/' . Str::slug($sliderItem['name'] ?? $sliderItem['title']) . '-' . $sliderItem['id']) ?>"
