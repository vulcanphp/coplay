<a fire :href="video.type + '/-' + video.id">
    <template x-if="video.poster">
        <img :src="'https://image.tmdb.org/t/p/w185' + video.poster" alt="poster" class="hover:opacity-75 w-full rounded-md transition ease-in-out duration-150">
    </template>
    <template x-if="!video.poster">
        <div class="text-center text-gray-400 opacity-25">
...
<div class="mt-2">
    <a fire :href="video.type + '/-' + video.id" class="mt-2 hover:text-gray-300" x-text="video.title + ' ('+ (video.type == 'movie' ? 'Movie' : 'Tv') +')'"></a>
</div>