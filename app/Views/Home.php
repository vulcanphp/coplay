...
class Home
{
     ...
     * @return Response The rendered search result page.
     */
    public function search(Request $request)
    {
        $results = [];
        $keyword = trim($request->query('keyword')); // get the search keyword
            ...
            try {
                $results = collect(
                    ...
                    ->map(function ($vid) use ($keyword, $year) {
                        ...
                        $priority += (slugify($vid['title'] ?? $vid['name']) == slugify($keyword)) ? 500 : 0;
                ...
                return response()->write('Internal Server Error, Please Try again later.');
            }
        } else {
