<section class="text-center mt-4">
    ...
    <div x-cloak x-show="tabOpen == 'movie'" class="mt-6">
        <input type="text" @change="initFrame()" x-model="tmdb"
            class="w-4/12 text-center p-2 bg-primary-900 border-2 font-semibold border-primary-800 rounded-sm outline-none focus:border-accent-400 text-primary-200">
    </div>
    ...
    <div class="mt-4">
        <input type="text" readonly x-model="frame"
            class="px-4 text-center py-2 bg-primary-900 w-full border-2 focus:border-accent-400 font-semibold border-primary-800 rounded-sm outline-none text-primary-200">
        <iframe :src="frame" class="w-full rounded-sm bg-primary-900 h-64 md:h-72 lg:h-80 xl:h-96 mt-4"
