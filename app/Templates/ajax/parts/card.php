...
?>
<a href="<?= $slug ?>"
    class="rounded hover:text-primary-300 focus:text-primary-200 focus:outline-none focus:ring-2 focus:ring-accent-500 px-1 py-2 <?= $index > 0 ? 'border-t border-primary-700' : '' ?> flex items-center">
    <?php if (isset($video['poster_path'])): ?>
        <img src="https://image.tmdb.org/t/p/w92<?= $video['poster_path'] ?>" class="w-3/12 object-cover rounded"
            alt="poster">
    <?php else: ?>
